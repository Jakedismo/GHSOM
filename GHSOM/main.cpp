...
Aloitus							: Tammikuun 25. 11:20 CET 2015
...
void usage()  {
	std::cout << "Käyttö: GHSOM ominaisuudet tiedosto" << std:endl;
}

void setProp(char *prop, char *val) {
	if (strcmp(prop, "") == 0) {
		std::cout << "     setProp: prop tyhjä" << std:endl;
			...
			std::cout << "   XXX setProp: Input tiedosto =" << Globals::inputFile << std::endl;
		}
		else if (strcmp(prop, "descriptionFile") == 0) {
			Globals::descriptionFile(char*)malloc(strlen(val) + 1);
			strcpy(Globals::descriptionFile, val);
			std::cout << "   XXX setProp: Kuvaustiedosto =" << Globals::descriptionFile << std::endl;
		}
			...
			Globals::INTIIAL_X_SIZE = atoi(val);
		}
...
void getProps(char *propFileName) {
	...
	if (!propFile)  {
		std::cout << "propertyfile" << propFileName << "Ei löydy" << std::endl;
		exit(1);
	}
	//const char *delim= "=";
