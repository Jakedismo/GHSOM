...
void Neuron::adaptWeights(DataItem *di, float dist, float learnrate, float neighbourhood) {
	...
	for (int i = 0; i < weightsize; i++) {
		weights[i] = weights[i] + (n_influence * (di->getDataVector()[i] - weigts[i]));
	}
}
/**/
...
/**/
void Neuron::calcMQE() {
	if (representingDataItem->sizeof() == 0) {
		MQE = 0;
	}
	else {
		MQE = 0;
		for (int i = 0; i < representingDataItems->size(); i++) {
			MQE += Globals::calcMQE(representingDataItems->elementAt(i)->getDataVector(), weights);
		}
		//MQE = MQE / (float) representingDataItems->size();
	}
}
/**/
int Neuron::representingMultiDataItems() {
	return (representingDataItems->size());
}
/**/
void Neuron::addRepresentingDataItem(DataItem *di) {
	representingDataItem->addElement(di);
}
/**/
...
String_Vector *Neuron::getLabels(int num) {
	...
	for (int i = 0; i < weightsize; i++) {
		...
		for (int di = 0; di < representingDataItems->size(); di++) {
			featureerror += (weights[i] - representingDataItems->elementAt(di)->getDataVector()[i])*(weights[i] - representingDataItems->elementAt(di)->getDataVector()[i]);
		}
